...
#### Assay Pack Table
- **타입**: 테이블 (5개 컬럼)
- **컬럼**: Assay Pack / Category / Compatible / Version / Date
- **선택**: 행 클릭으로 선택
...
- **옵션**:
  ...
  - Optical Film
  - Adhesive Seal
- **기본값**: 첫 번째 옵션 (Standard Cap) 자동 선택

#### Plate/Tube Category
- **타입**: 라디오 버튼 그룹
- **옵션**:
  - 96-well Plate (Standard)
  - 96-well Plate (Low Profile)
  - 384-well Plate (Standard)
  - 384-well Plate (Low Profile)
  - 8-tube Strip
  - PCR Tube (0.2ml)
  - PCR Tube (0.5ml)
- **기본값**: 첫 번째 옵션 (96-well Plate Standard) 자동 선택
- **Plate 타입 변경**: 선택된 옵션에 따라 Plate Grid가 96-well 또는 384-well로 자동 변경
...
#### Section Header
- **제목**: "Plate Setting"
- **우측 영역**: Plate 타입 토글 버튼 (96-well / 384-well)
  - **토글 버튼**: 두 개의 버튼으로 구성
  - **96-well**: 기본 선택 상태
  - **384-well**: 비활성 상태
  - **전환 시 동작**: Assay가 선택되어 있으면 확인 팝업 표시, Plate 타입 변경 시 Plate 데이터 초기화 후 Base Assay로 재설정

#### Instruction Text
- **초기 상태**: "Select an assay pack to start"
- **Assay 선택 후**: "Base assay: [Assay Name]. Select an assay to paint on plate."
- **브러시 선택 시**: "Click or drag on wells to apply: [Assay Name]"
...
- **Well 크기**: 18px × 18px (크기 축소)
- **Well 간격**: 2px

**공통 Well 스타일**:
- 기본: 흰색 배경, #9CA3AF 테두리
- Hover: 테두리 굵기 증가, 색상 #1F2937
- 적용된 Assay: 해당 Assay 색상 배경

**초기 상태**:
- 페이지 로드 시: 96-well과 384-well 모두 렌더링되어 있음
- 96-well이 기본 표시, 384-well은 숨김 상태
- 토글 버튼 클릭 시 표시/숨김 전환

#### 인터랙션
1. **Base Assay 적용**: Assay Pack 선택 시 전체 Well이 자동으로 해당 Assay로 채워짐
2. **Single Click**: 브러시로 선택된 Assay를 클릭한 Well에 적용
3. **Drag Selection**: 마우스 드래그로 여러 Well 선택하여 브러시 Assay 일괄 적용
...
- **User**: 버튼 자체가 표시되지 않음 (`display: none`)
...
### 6.1 Start Analysis 검증

1. **Assay Pack 선택 여부**
   - 조건: Base Assay 선택되지 않음
   - 동작: 경고 메시지 표시

2. **Consumables 선택 여부**
   - 조건: Cap/Film 또는 Plate/Tube 미선택
   - 동작: 경고 메시지 표시

### 6.2 Save as Preset 검증
...
5. Cap/Film, Plate/Tube 기본값 자동 선택
6. Plate 초기화
...
3. Plate 데이터 업데이트
4. Additional Assay인 경우 selectedCombinationAssays에 추가
5. Legend 업데이트
...
**동작**:
1. 브러시 선택 확인 (미선택 시 경고 메시지)
2. mousedown 이벤트에서 시작 좌표 저장
3. mousemove 이벤트 동안 현재 좌표 추적
4. mouseup 이벤트에서 드래그 영역 내의 모든 Well 선택
5. 선택된 모든 Well에 브러시 Assay 색상 적용
6. Plate 데이터 업데이트
7. Additional Assay인 경우 selectedCombinationAssays에 추가
8. Legend 업데이트
...
2. 확인 선택 시:
   ...
   - Plate 데이터 업데이트
   - selectedCombinationAssays에서 제거
   - Combination List 항목 상태 업데이트 (삭제 버튼 숨김)
   - 활성 상태 제거
   - 브러시가 삭제된 Assay였다면 브러시 해제
   - Legend 업데이트
   - Summary 업데이트
   - Instruction 텍스트 업데이트

### 7.7 Base Assay 삭제 (전체 초기화)
...
### 7.11 Plate 타입 전환 (96-well ↔ 384-well)

**트리거**:
- Plate Setting Section 헤더의 토글 버튼 클릭

**사전 조건**:
- 없음 (Assay 선택 여부와 무관)

**동작**:
1. Assay가 선택되어 있으면 확인 팝업 표시
   - 메시지: "Changing plate type will reset the current configuration. Continue?"
   - 취소 선택 시: 현재 상태 유지
2. 확인 선택 시:
   - 토글 버튼 상태 업데이트 (클릭한 버튼 활성화)
   - 현재 Plate 타입 변경
   - 선택한 Plate 컨테이너 표시, 다른 Plate 숨김
   - Assay가 선택되어 있는 경우:
     - selectedCombinationAssays 배열 초기화
     - currentBrushAssay 초기화
     - Combination List 시각적 상태 초기화 (Base Assay 제외)
     - Plate 데이터 초기화 (선택한 타입에 맞게)
     - 전체 Plate를 Base Assay로 재설정
     - Legend 업데이트 (Base Assay만 표시)
     - Combination Summary 업데이트
     - Instruction 텍스트 업데이트
   - 이벤트 리스너 업데이트

**완료 후**:
- 선택한 Plate 타입으로 변경되고 Plate가 깨끗한 상태로 초기화됨
...
5. Cap/Film 첫 번째 옵션 자동 선택 확인
6. Plate/Tube 첫 번째 옵션 자동 선택 확인
...
2. 분석 데이터 구성 확인
3. 백엔드로 데이터 전송 확인 (콘솔 로그 또는 네트워크 확인)
4. 팝업 닫힘 확인
...
#### TC-PLATETYPE-001: 초기 상태에서 Plate 타입 전환
**권한**: 모든 사용자
**테스트 단계**:
1. Manual 팝업 열기
2. 96-well 토글 버튼 활성 상태 확인
3. 96-well Plate Grid 표시 확인 (8 rows × 12 columns)
4. 384-well 토글 버튼 클릭
5. 확인 팝업 표시 없이 바로 전환 확인
6. 384-well 토글 버튼 활성 상태 확인
7. 384-well Plate Grid 표시 확인 (16 rows × 24 columns)
8. 96-well Plate Grid 숨김 확인
9. 다시 96-well 토글 버튼 클릭
10. 96-well Plate Grid 표시 확인

#### TC-PLATETYPE-002: Assay 선택 후 Plate 타입 전환
**권한**: 모든 사용자
**전제조건**: Assay Pack 선택됨
**테스트 단계**:
1. Assay Pack 선택
2. 96-well Plate가 Base Assay 색상으로 채워짐 확인
3. 384-well 토글 버튼 클릭
4. 확인 팝업 표시 확인 ("Changing plate type will reset...")
5. 확인 클릭
6. 384-well Plate Grid 표시 확인
7. 384-well Plate가 Base Assay 색상으로 채워짐 확인 (384개 well)
8. Legend에 Base Assay만 표시 확인

#### TC-PLATETYPE-003: Additional Assay 적용 후 Plate 타입 전환
**권한**: 모든 사용자
**전제조건**: Assay Pack 선택됨, 2개의 Additional Assay 적용됨
**테스트 단계**:
1. Assay Pack 선택
2. Additional Assay 1 선택하여 Well A1-A6에 적용
3. Additional Assay 2 선택하여 Well B1-B6에 적용
4. Legend에 3개 Assay 표시 확인 (Base + 2 Additional)
5. 384-well 토글 버튼 클릭
6. 확인 팝업 표시 확인
7. 취소 클릭
8. 현재 상태 유지 확인 (96-well, Additional Assays 유지)
9. 다시 384-well 토글 버튼 클릭
10. 확인 클릭
11. 384-well Plate Grid 표시 확인
12. Plate가 Base Assay 색상으로만 채워짐 확인 (Additional Assays 제거됨)
13. Legend에 Base Assay만 표시 확인
14. Combination List의 Additional Assay 항목들 비활성 상태 확인
15. Summary 업데이트 확인 (Base Assay만)

#### TC-PLATETYPE-004: 384-well에서 Additional Assay 적용 후 96-well로 전환
**권한**: 모든 사용자
**전제조건**: Assay Pack 선택됨
**테스트 단계**:
1. Assay Pack 선택
2. 384-well 토글 버튼 클릭 (확인)
3. Additional Assay 1 선택하여 Well A1-A12에 적용
4. Additional Assay 2 선택하여 Well B1-B12에 적용
5. Legend에 3개 Assay 표시 확인
6. 96-well 토글 버튼 클릭
7. 확인 팝업 표시 확인
8. 확인 클릭
9. 96-well Plate Grid 표시 확인
10. Plate가 Base Assay 색상으로만 채워짐 확인 (96개 well)
11. Legend에 Base Assay만 표시 확인
12. Combination Summary 업데이트 확인
...
| 1.1 | 2026-01-20 | 96-well/384-well 토글 기능 추가, Plate 타입 전환 인터랙션 및 테스트 시나리오 추가 |
